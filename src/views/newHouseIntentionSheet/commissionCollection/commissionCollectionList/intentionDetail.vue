        ...
        <div class="g_content">
            <div class="g_detailAll">
                <div class="g_lineNav">
                    ...
                    v-if="!!detailInfo.annexList && detailInfo.annexList.length > 0"
                ...
                <div class="g_select" v-if="navSelectIndex == 1">
                    <div class="g_detailInfo">
                        <div class="infor">
                            <infor :projectDetailInfo="detailInfo"></infor>
                        </div>
                    </div>
                    <div class="g_statu">
                      <newReportNo :detailInfo="detailInfo"></newReportNo>
                    </div>
                </div>
                <div class="g_select" v-if="navSelectIndex == 2">
                    <chargeRecords :detailList="receiptPaymentList"></chargeRecords>
                </div>
                <div class="g_select" v-if="navSelectIndex == 3">
                    <Annex :detailInfo="detailInfo.annexList"></Annex>
                </div>
            </div>
            <!--右上角缩小关闭按钮-->
...
import {
    GetIntentionDetail,
    GetIntentionChargeRecordsList
} from "../../../../net/newHouseIntentionSheet/intentionSheet/intentionDetail"  
...
export default {
    ...
    methods:{
        initData(){
            ...
            // 获取新房意向单详情(包括附件详情)
            new GetIntentionDetail(parameter).send()
            .then((res)=>{
                console.log("获取新房意向单详情:",res);
                this.detailInfo = res;
            })
            .catch((err)=>{
            ...
            //获取获取新房意向单收付记录列表
            new GetIntentionChargeRecordsList(parameter).send()
            .then((res)=>{
                console.log("获取获取新房意向单收付记录列表:",res);
                this.receiptPaymentList = res;
            })
            .catch((err)=>{
                console.log('服务器开了个小差，重稍后重试');
            })
