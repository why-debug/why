    ...
    <div class="void">
      ...
      <div class="g_photographerName">
        <div class="g_photographerName_item">
          ...
          <div class="u_title">{{'张三'}}</div>
        </div>
        <div class="g_photographerName_item">
          <div class="u_title caozuo">申请编号:</div>
          <div class="u_title bianhao">{{'FSDFHFAFJALFAFHNA'}}</div>
        </div>
      </div>
      <div class="agent_input">
        <div class="must">作废时间：</div>
        <el-date-picker v-model="params.cancelDate" type="date" value-format="yyyy-MM-dd" placeholder="选择日期"></el-date-picker>
      </div>
      <div class="agent_input">
        <div class="must">作废原因：</div>
        <el-select v-model="params.invalidReason" placeholder="请选择作废原因">
          <el-option
      ...
      <div class="agent_input" v-if="params.invalidReason === 0">
        <div class="must">其他原因：</div>
        <el-input v-model="params.reason" placeholder="请输入其他原因"></el-input>
      </div>
...
<script>
import { getCancellation } from "../../../../net/projectAgentinfo/distributionContract/cancellation.js";
import { ErpCommon } from "../../../../utils/ErpCommon";
export default {
  data() {
    return {
      reasonList: [
        { label: "美联房遗失合同", value: 1 },
        { label: "开发商遗失合同", value: 2 },
        { label: "开发商修改条款", value: 3 },
        { label: "其他原因", value: 0 }
      ],
      verified: true,
      stopTime: "", //终止时间
  ...
  methods: {
    ...
    saveData() {
      ...
      new getCancellation(this.params)
        .send()
        .then(res => {
          console.log(res)
        })
    ...
    checked() {
      this.verified = true;
      if (this.params.cancelDate === "") {
        this.verified = false;
        new ErpCommon().toast("请选择作废时间");
        return
      }
      if (this.params.invalidReason === "") {
        this.verified = false;
        new ErpCommon().toast("请选择作废原因");
      }
      if (this.params.invalidReason === 0 && this.params.reason === "") {
        this.verified = false;
        new ErpCommon().toast("请输入其他原因");
      }
    },
    //关闭弹窗
    closeView() {
