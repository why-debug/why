    ...
    <div class="guidang">
      ...
      <div class="g_photographerName">
        <div class="u_title must">我的意见:</div>
          <el-radio v-model="params.isAgree" :label="1">同意</el-radio>
          <el-radio v-model="params.isAgree" :label="0">不同意</el-radio>
      </div>
      ...
      <div class="agent_input">
        <div>预计归还日期：</div>
        <!-- <el-date-picker v-model="backTime" type="date" placeholder="选择日期"></el-date-picker> -->
        <div>2012-12-12</div>
      </div>
...
import {getcontrackApprove} from "../../../../net/projectAgentinfo/distributionContract/contrackApprove"
...
export default {
  data() {
    return {
      ...
      params:{
        contractRemark: "", //(string)批借备注  若选择不同意则必填
        dealId: "", //(int32)要批借的合同ID
        isAgree: 1, //(byte)我的意见： 0：不同意  1：同意
        returnTime: ""//(date-time)预计归还日期
      }
    };
  },
  props: {
  ...
  methods: {
    ...
    saveData() {      
      ...
      new getcontrackApprove(this.params)
      .send()
      .then(res=>{
        console.log(res.data);
      })
      .catch(res=>{
            new ErpCommon().toast("操作失败,请稍后再试");
      })
