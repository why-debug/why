      ...
      <div v-show="showTc" class="select_tc f_c_s">
        ...
        <div class="area_tc_cont" @click.stop="">
          ...
          <div class="f_c_c input_view">
            <div>
              <input @input="inputChange" v-model="inputVal" autocomplete="off" placeholder="请输入部门名称..." />
            </div>
          </div>
          ...
          <div v-show="!searchList" class="area_cont">
            ...
            <div v-for="(item, index) in list"
              ...
              <div class="f_r_s" style="align-items: center">
                ...
                <div v-if="isdisplaypeopleNum"> ({{orgPeopleNumMap.get(item.id) || 0}}人)</div>
              </div>
              <div class="g_rightBtn">
          ...
          <div v-show="searchList && searchList.length > 0" class="area_cont">
            <div v-for="(item, index) in searchList"
                 @click.stop="clickSearchItem(item, -1)"
                 class="select_item_click f_r_b"
                 :key="index">
              <div class="f_r_s" style="align-items: center">
                <div class="max_org_name">{{ item[searchItemName] }}</div>
                <div v-if="isdisplaypeopleNum"> ({{orgPeopleNumMap.get(item.organizationId) || 0}}人)</div>
              </div>
            </div>
          </div>
  ...
  export default {
    data() {
      return {
        ...
        // 总数
        allPersonNum: 0,
        isdisplaypeopleNum:true,
        Reset:false,
        searchItemName:"organizationName",
        isloading:false,
      }
    },
    props:{
    ...
    methods: {
      ...
      chooseInfo() {
        this.clickShow = true;
        this.showTc = true;
        // this.searchList = null;//每次点击之后显示人数
      },
      ...
      inputChange() {
        ...
        this.searchItemName = "organizationName";
      ...
      clickItem(item,index) {
        if(this.isloading){
          this.$erpCommon.toast("正在加载员工数据,请勿重复操作",'error');
          return
        }
        this.isdisplaypeopleNum = true;
        if(item.isOrg){
          this.f_selectStaff(item,index,false);
          return
        }
      ...
      //递归查找部门
      f_callbackSelectOrg(list,id){

        for (let item of list) {
          if(item.id == id){
            return item;
          }
          if( !!this.f_callbackSelectOrg(item.children,id) ){
            return this.f_callbackSelectOrg(item.children,id);
          }
        }
        return ''
      },
      // 搜索点击
      clickSearchItem(val,index) {
        let itemVal = this.f_callbackSelectOrg(PrivateData.treeData,val.organizationId);
        let item = !itemVal ? val : itemVal;
        if(item.isOrg){
          this.isdisplaypeopleNum = true;
          this.f_selectStaff(item,index,true);
          return
        }
      ...
      //选择员工
      f_selectStaff(item,index,isSearch){
        this.isloading = true;
        new GetProjectLeaderList(new GetProjectLeaderListRequest(item.id)).send()
        .then(res=>{
          res.forEach(element => {
            element.isOrg = false;
          });
          PrivateData.allListStaff = res;
          if(isSearch){
            this.inputVal = '';
          ...
          this.isdisplaypeopleNum = false;

          if(!isSearch){
            this.nextOrg(item,index);
          }
          this.isloading = false;
        }).catch(error=>{
          this.isloading = false;
          console.error("搜索员工时报错",error);
        })
      },

      // 切换下级
      nextOrg(item, index) {
        if(!this.isdisplaypeopleNum){
          this.navList.push({
            name: item.name+"-员工",
            id: item.id
          });
  ...
  .select_tc{
    ...
    .area_tc_cont{
      ...
      .select_item_click{
        ...
        &:hover{
          background-color: @inputBoxShadowColor;
          color: #ffffff;
          .org_icon{
