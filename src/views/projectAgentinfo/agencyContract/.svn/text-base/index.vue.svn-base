    ...
    <div class="order">
      ...
      <div class="g_topselect">
        <div class="g_selectMenu">
          ...
          <div class="u_select">
            <selectRadio
              @onSelectChange="f_selectTimeType"
              :list="timeList"
              :defaultText="'时间类型'"
              :empty="empty"
              imgSrc="type_200"
        ...
        <div class="right_box">
          ...
          <div class="topseach" @click.stop="filterPoput">
            <img
              ...
              class="addIcon"
            />
            <div class="filterChocie">
              筛选：
              <newAgentTopFilter
                ref="filterPoput"
                @filterParams="f_filterParams"
                :empty="false"
              ></newAgentTopFilter>
            </div>
            <span class="shaixuan_value" :title="filterDefault || '默认'">{{
              filterDefault || "默认"
            }}</span>
          </div>
          <div
            class="topseach"
            @click.stop="f_exportClick"
            v-if="haveExportAuth"
          >
            <img
        ...
        <div style="width: 100%;height: 100%;overflow: auto">
          ...
          >
            ...
            >
              ...
              <div class="list_item time_shenpi" :title="item.approvalTime">
                {{ item.approvalTime }}
              </div>
              <div
              ...
              <div class="list_item shenqing_person" :title="item.applicant">
                {{ item.applicant }}
              </div>
              <div
              ...
              <div class="list_item project_name" :title="item.projectName">
                {{ item.projectName }}
              </div>
              <div
                class="list_item agent_time"
                :title="item.agencyTermStart + '至' + item.agencyTermEnd"
              >
                {{ item.agencyTermStart | formatTime }}至{{
                  item.agencyTermEnd | formatTime
                }}
              </div>
              <div
      ...
      <Termination
        ...
        :title="`代理合同终止`"
        :dealId="dealId"
        :operationPersonParams="statusParams"
      ></Termination>
      <Void
        v-if="toChange === 4"
        @closeView="closeView"
...
import {
  ...
  agentListRequest,
} from "../../../net/projectAgentinfo/agencyContract/agencyContractList.js";
import {
  GetUserOpers,
  GetUserOpersRequest,
...
export default {
  ...
  methods: {
    ...
    async initUserOpers() {
      ...
      this.haveExportAuth = this.$UserPermission.AGENCYContractExportAuth();
      this.haveEditAuth = this.$UserPermission.AGENCYContractUpdateAuth();
      this.haveAddAuth = this.$UserPermission.AGENCYContractAddAuth();
    ...
    initData() {
      ...
      new getAgencyContractList(new agentListRequest(this.params))
        ...
        .catch((res) => {
          this.isLoad = false;
          this.$erpCommon.toast("服务器开小差了,请稍后再试", "error");
        });
    },
    //新增代理合同
    toAddContract() {
    ...
    async f_exportClick() {
      ...
      // 导出接口
      let data = await new getAgencyContractList(
        new agentListRequest(exportParam)
      )
        .send()
        .then((res) => {
          return res.list;
