  ...
  <div class="add">
    ...
    <div class="content">
      <div class="middle_content">
        ...
        <contractSignF ref="getSigningA" @contractSigning="getSigningF" :pageIndex="pageIndex"  :initParams="editParams"></contractSignF>
        <!-- <contractSignT ref="getSigning" @contractSigning="getSigning" :pageIndex="pageIndex"  :initParams="editParams"></contractSignT> -->
        <contractSigningB ref="getSigningB" @contractSigning="getSigningB" :pageIndex="pageIndex"  :initParams="editParams"></contractSigningB>
...
import contractSignF from "./addContract/contractSignF";
import contractSignT from "./addContract/contractSignT";
...
import { getAddAgent } from "../../../net/projectAgentinfo/agencyContract/addAgentContract";
import { GetContractInfoDetailsList } from "../../../net/projectAgentinfo/agencyContract/InitContractInfoDetails";
export default {
  components: {
    BasicMessage,
    contractSignF,
  ...
  methods: {
    ...
    save() {
      ...
      this.$refs.getSigningA.sendSigningF();
      this.$refs.getSigningB.sendSigningB();
   ...
   getSigningB(val) {
      ...
      delete val.data.dealConfirmation;
      delete val.data.forbid;
      delete val.data.distributionCompanyName;
    ...
    f_getInspectParameterValue(el){
        ...
        if(el.agencyScope ===""){
            this.$erpCommon.toast("请选择代理范围",'error');
            return false;
        }
         if(!el.dealName){
        ...
        if(!el.agencyBusiness){
            this.$erpCommon.toast("请选择代理业务",'error');
            return false;
        }
       ...
       if(el.stampType.indexOf('3') != -1){
         ...
         if(el.partyADockingPersonMobile !=''){
           if(!this.judgeTel(el.partyADockingPersonMobile,'甲方对接人')) return false;
        }
        ...
        for(let info of el.personInChargeList){
         ...
         if(!el.agencyTermType){
            this.$erpCommon.toast("请选择代理期限类型",'error');
            return false;
        }else if(el.agencyTermType ==1){
          if(el.agencyTermStartStr == '' || el.agencyTermEndStr == ''){
            this.$erpCommon.toast("请选择代理期限时间",'error');
               return false
          }
        }

          if(!el.commissionStandard){
            this.$erpCommon.toast("请选择结佣标准",'error');
            return false;
        }else if(el.commissionStandard ==1){
          if(el.commissionStandardList<1){
            this.$erpCommon.toast("请选择新房代理类型",'error');
               return false
          }
        }
