<template>
  ...
  <div class="add-deal-report">
    ...
    ></choose-property>
    <!-- 选择片区 -->
    <select-area @sectionName="sectionName" :isShow.sync="selectAreaObj.isShow"></select-area>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    if (this.standardCommission) {
      console.log(this.standardCommission);
  ...
  methods: {
    ...
    selsecPerson(data) {
      ...
      // this.ruleForm.erpMlDealProfits[i].userName = info.userName;
    ...
    // 选择所属片区回显内容
    sectionName(v) {
      this.ruleForm.erpMlDeal.houseReg = v.value; //传递请求参数
      this.houseRegName = v.label; //页面显示内容
      console.log(v);
    },
    // 合作费改变
