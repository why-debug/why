    ...
    <main>
      ...
      <section>
        <project_info
          ...
          :initData="newHouseDetailInfo"
        />
      </section>
      <section>
        <subscription_info
          ref="getSubscriptionInfo"
...
export default {
  ...
  created() {
    //获取url 是否存在合同id
    let reportId = this.$route.query.reportId || ""; //url参数
    if (reportId) this.initData(reportId);
  },
  methods: {
    //若为编辑则初始化详情数据
    initData(reportId) {
    ...
    //保存成交报告
    save() {
      let flag1 = this.$refs.getProjectInfo.projectInfoCheck();
      if (flag1) {
        let flag2 = this.$refs.getSubscriptionInfo.subscriptionInfoCheck();
        if (flag2) {
          let flag3 = this.$refs.getCusInfo.custInfoCheck();
          if (flag3) {
            let flag4 = this.$refs.getPerformanceInfo.performanceCheck();
            if (flag4) {
              this.saveParams = {};
    ...
    submit() {
      //检查元素
      let addParams = JSON.parse(JSON.stringify(this.saveParams));
      console.log(this.saveParams, "提交的参数", addParams, "==============");
