...
class RequestApi extends PostRequest {
  ...
  getUrl() {
    return "/newBuildWeb/broker/build/queryDistributionContractDetails";
  ...
  send() {
    ...
    return new RequestApi(this.request).send()
      .then((res) => {
        ...
        item.agencyScope = list.agencyScope || item.getDefault();
        ...
        item.distributionCompanyName = list.distributionCompanyName || item.getDefault();
